...
class SearchOpenOpusWindow(ctk.CTk):
    def __init__(self, user_conector: UserConector, user: User, composer_query: str ,query: str):
        super().__init__()

        self.user_conector = user_conector
        self.work_connector = WorkConector()
        self.query: str = query
        self.composer_query: str = composer_query
        self.user = user
    ...
    def build_works(self):
        try:
            works: list[Work] = self.work_connector.search_work_openopus(composer_query=self.composer_query, query=self.query)

            if not works:
                messagebox.showerror("No works found rewrite your query")
