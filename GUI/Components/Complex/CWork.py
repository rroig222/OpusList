...
class CWork(ctk.CTkFrame):
    def __init__(self, master, work: Work, user: User):
        ...
        self.rating = UserConector().get_rate(work, user)
    ...
    def create(self):
        ...
        # NUEVA etiqueta de rating
        self.rating_label = ctk.CTkLabel(
            self.left_frame, text=f"Rating: {self.rating}", text_color="black"
        )
        self.rating_label.grid(row=4, column=0, sticky="w", pady=(5, 0))
